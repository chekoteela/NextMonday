...
public class Thursday implements WriteToDB {
    public static final String TODAY = THURSDAY;
    ...
    public void writeToDB(TargetDiary targetDiary, Map<String, Boolean> mapRepeats, TargetDataService service, Calendar calendar) {
        ...
        if (Calendar.getInstance().get(Calendar.DAY_OF_WEEK) == Calendar.THURSDAY && Objects.requireNonNull(mapRepeats.get(TODAY)).equals(true)) {
        ...
        if (Calendar.getInstance().getTimeInMillis() + 604800000 * REPEAT_WEEK > calendar.getTimeInMillis()) {
            new Friday().writeToDB(targetDiary, mapRepeats, service, calendar);
        }
    }
}
