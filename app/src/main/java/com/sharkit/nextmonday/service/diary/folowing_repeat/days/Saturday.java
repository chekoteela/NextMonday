...
public class Saturday implements WriteToDB {
    public static final String TODAY = SATURDAY;
    ...
    public void writeToDB(TargetDiary targetDiary, Map<String, Boolean> mapRepeats, TargetDataService service, Calendar calendar) {
        ...
        if (Calendar.getInstance().get(Calendar.DAY_OF_WEEK) == Calendar.SATURDAY && Objects.requireNonNull(mapRepeats.get(TODAY)).equals(true)) {
        ...
        if (Calendar.getInstance().getTimeInMillis() + 604800000 * REPEAT_WEEK > calendar.getTimeInMillis()) {
            new Sunday().writeToDB(targetDiary, mapRepeats, service, calendar);
        }
    }
}
