...
public class Thursday implements WriteToDB {
    @Override
    public void writeToDB(MyTarget target, Map<String, Boolean> mapRepeat, TargetData targetData, Calendar calendar) {
        ...
        Log.d("TAGA", dateFormat.format(calendar.getTimeInMillis()) + " thursday");

        if (Calendar.getInstance().get(Calendar.DAY_OF_WEEK) == Calendar.THURSDAY && mapRepeat.get("Thursday").equals(true)) {
        ...
        } else if (mapRepeat.get("Thursday").equals(true)) {
        ...
        if (Calendar.getInstance().getTimeInMillis() + 604800000 > calendar.getTimeInMillis()) {
            new Friday().writeToDB(target, mapRepeat, targetData, calendar);
        }
    }
}
