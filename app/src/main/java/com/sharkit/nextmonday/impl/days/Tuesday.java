...
public class Tuesday implements WriteToDB {
    @Override
    public void writeToDB(MyTarget target, Map<String, Boolean> mapRepeat, TargetData targetData, Calendar calendar) {
        ...
        Log.d("TAGA", dateFormat.format(calendar.getTimeInMillis()) + " tuesday");

        if (Calendar.getInstance().get(Calendar.DAY_OF_WEEK) == Calendar.TUESDAY && mapRepeat.get("Tuesday").equals(true)) {
        ...
        } else if (mapRepeat.get("Tuesday").equals(true)) {
        ...
        if (Calendar.getInstance().getTimeInMillis() + 518400000 > calendar.getTimeInMillis()) {
            new Wednesday().writeToDB(target, mapRepeat, targetData, calendar);
        }
    }
}
