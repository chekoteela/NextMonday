...
public class Sunday implements WriteToDB {
    ...
    public void writeToDB(MyTarget target, Map<String, Boolean> mapRepeat, TargetData targetData, Calendar calendar) {
        ...
        Log.d("TAGA", dateFormat.format(calendar.getTimeInMillis()) + " sunday");

        if (Calendar.getInstance().get(Calendar.DAY_OF_WEEK) == Calendar.SUNDAY && mapRepeat.get("Sunday").equals(true)) {
        ...
        } else if (mapRepeat.get("Sunday").equals(true)) {
