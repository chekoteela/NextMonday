...
public class Saturday implements WriteToDB {
    @Override
    public void writeToDB(MyTarget target, Map<String, Boolean> mapRepeat, TargetData targetData, Calendar calendar) {
        ...
        Log.d("TAGA", dateFormat.format(calendar.getTimeInMillis()) + " saturday");

        if (Calendar.getInstance().get(Calendar.DAY_OF_WEEK) == Calendar.SATURDAY && mapRepeat.get("Saturday").equals(true)) {
        ...
        } else if (mapRepeat.get("Saturday").equals(true)) {
        ...
        if (Calendar.getInstance().getTimeInMillis() + 604800000 > calendar.getTimeInMillis()) {
            new Sunday().writeToDB(target, mapRepeat, targetData, calendar);
        }
    }
